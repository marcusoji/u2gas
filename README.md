...
```
cd web
cp .env.example .env          # then set VITE_USE_MOCKS=true
npm ci
npm run dev
```

A demo bar appears bottom-right. It switches between the four apps, because
each is behind a role gate a real deployment authorises on the server. You can
also link straight to one:
...
envelope and error codes as the Worker, so nothing above `api.ts` knows the
difference. Product images come from `public/mock-media/`, regenerated by
`scripts/gen-mock-images.py`. With `VITE_USE_MOCKS` unset the mock module is
inert and the app talks to the real API.
